...
# Datapack Guide
### NOTE: This guide is for 1.21.1 Fabric!
*For information on NeoForge or other versions, make sure to select the appropriate branch!*
...
**Current Providers:**
- Puffish's Skills
- Playtime-based scaling (configurable)

---

## Loot Settings

Dynamic Difficulty provides a flexible system for level-based loot drops using Fabric's loot table mixin system.
...
**Note:** This config option toggles the entire built-in loot injection system. When disabled, no loot injection occurs regardless of what's in the loot table. If you want to keep the system active but customize the drops, override the loot table instead (see [Overriding the Built-in Loot](#overriding-the-built-in-loot)).
...
To customize or replace the built-in loot behavior, you can override the loot table:

**Replace the loot table**

Create your own loot table at `data/dynamic_difficulty/loot_tables/inject/level_based_drops.json` in your datapack. The built-in mixin will automatically use your table instead.
